# Incognito Nano App tests

These tests are developed from the boilerplate framework https://github.com/LedgerHQ/app-boilerplate
...
cd <your app repository>                        # replace <appname> with the name of your app, (eg incognito)
...
cp bin/app.elf tests/elfs/<appname>_<device>.elf    # replace <device> with one of [nanos, nanox, nanosp]
                                                    # replace <appname> with the name of your app, (eg incognito)
                                                    # so for example tests/elfs/incognito_nanos.elf
```
...
```
docker pull ghcr.io/ledgerhq/ledger-app-builder/ledger-app-builder-lite:latest
cd app-<appname>/                                   # replace <appname> with the name of your app, (eg incognito)
docker run --user "$(id -u)":"$(id -g)" --rm -ti -v "$(realpath .):/app" --privileged -v "/dev/bus/usb:/dev/bus/usb" ledger-app-builder-lite:latest
