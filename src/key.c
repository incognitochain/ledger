...
// G_crypto_state_t.a
void incognito_gen_private_view_key(unsigned char *key)
{
  incognito_keccak_F(G_crypto_state_t.key.key, 32, key);
  incognito_reduce(key, key);
}
...
void incognito_gen_public_view_key(unsigned char *key)
{
  ...
  incognito_gen_private_view_key(pkey);
  incognito_ecmul_G(key, pkey);
}
